<template>
  <div ref="TowerChart" :class="className" :style="{ height: height, width: width }" />
</template>
...
export default {
  ...
  methods: {
    ...
    addChartResizeListener() {
      ...
      instance.listenTo(this.$refs.TowerChart, () => {
    ...
    initChart() {
      this.chart = echarts.init(this.$refs.TowerChart);
      this.setOptions(this.chartData);
    },
    setOptions(val) {
      let that = this
      // 颜色
      const barColor1 = new echarts.graphic.LinearGradient(
        0, 0, 0, 1,
        [
      ...
      const barColor2 = new echarts.graphic.LinearGradient(
        0, 0, 0, 1,
        [
          {offset: 0, color: 'rgba(76, 204, 255, 1)'},
          {offset: 1, color: 'rgba(76, 204, 255, 0)'}
        ]
      );
        ...
        this.chart.setOption(
          {
            ...
            xAxis: {
              ...
              axisLabel: {
                formatter(val) {
                  return that.handleAxisLabel(val, 6)
                },
                // width: 80,
                // overflow: 'truncate',
                interval: 0,
